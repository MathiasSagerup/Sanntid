...
// jobben til denne er å være mellommann - den skal ta imot requests, og sende ressurser til brukere, og ta imot ressurser som blir gitt tilbake.
// her har RM aktiv rolle: holde kø og delegere
func resourceManager(askFor chan ResourceRequest, giveBack chan Resource) {

	res := Resource{}
	busy := false
	queue := PriorityQueue{} //RM har egen kø

	for {
		//Hvis ressurs er ledig og noen venter, send ressurs til bruker.
		if !busy && !queue.Empty() {
			request := queue.Front().(ResourceRequest)
			queue.PopFront()
			request.channel <- res
		...
		select {
		case request := <-askFor: //sett inn i kø etter prioritet
			queue.Insert(request, request.priority)
		case res = <-giveBack: //mottas noe tilbake - sett busy til false
			busy = false
		}
	}
}
