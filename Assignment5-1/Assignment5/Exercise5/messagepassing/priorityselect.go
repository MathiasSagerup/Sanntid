...
//RM er mer passiv her: den tilbyr ressurser på kanalner, og brukere tar den selv når de er klar 
//den bruker ikke kø, men select. 
func resourceManager(takeLow chan Resource, takeHigh chan Resource, giveBack chan Resource){

    res := Resource{} //manager har ressurs 

    for {
        //prøv først å gi til høy prio 
        select {
        case takeHigh <- res: //skjer hvis noen med høy prio venter
        default: //ingen høy prio klar? Vent på hvem som helst
            select {
            case takeHigh <- res:
            case takeLow <- res:
            }
        ...
        //
        res = <-giveBack //loop starter på nytt når ressurs gis tilbake 
    }
}
