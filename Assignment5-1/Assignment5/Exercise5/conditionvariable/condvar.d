...
class Resource(T) {
    ...
    T allocate(int id, int priority){ //når en id (med prioritet) vil ha ressurs
    mtx.lock(); //lås ressurs, så ingen endrer kø/ressurs samtidig som meg 
    
    queue.insert(id, priority); //tråd legger seg selv inn i felles kø for ressurs 
    
    while(queue.front() != id){ //hvis jeg ikke er foran i kø, vent (gi fra nøkkel) til min tur
        cond.wait();
    }
    //ta ressurs 
    auto v = value;
    ...
    //Når en er ferdig med ressurs 
    void deallocate(T v){
        mtx.lock(); //lås kød/ressurs 

        value = v; //oppdater ressurs
        queue.popFront(); //fjern denm som var ferdig 
        cond.notifyAll(); //alle kan sjekke om det er deres tur

        mtx.unlock(); //lås opp/ferdig 
    }
}
