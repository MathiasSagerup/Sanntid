...
procedure protectobj is
    ...
    -- ada gjør at vi kan sikre at en tråd får bruke ressurs om gangen
        -- HØYPRIORITET: Kan ta ressursen hvis den er ledig (not busy). Den skal få ressurs først. 
        entry allocateHigh(val: out IntVec.Vector) when not busy is
        begin
            busy := true;              -- Marker ressursen som opptatt
            val := value;              -- Gi ressursen til denne tåraden
        end allocateHigh;
    
        -- LAVPRIORITET: Kan bare ta ressursen hvis: Ressursen er ledig (not busy) OG Ingen høyprio venter (allocateHigh'Count = 0)
        -- lavprio venter på høyprio
        ...
        -- AVGI RESSURSEN: Når noen er ferdig med ressursen
        procedure deallocate(val: IntVec.Vector) is
        begin
            value := val;              -- Lagre oppdatert ressurs
            busy := false;             -- Marker som ledig, vekk neste i kø
        end deallocate;

    end Resource;

    -- Forventer å få ut kjøring: høy prio (partall) kjøres først, deretter oddetall 
