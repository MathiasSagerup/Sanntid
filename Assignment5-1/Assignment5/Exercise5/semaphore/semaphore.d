...
class Resource(T) {
    ...
    //Her skal vi selv styre hvem som får ressursen, manuelt 


    
    //låse tilgang til ressurs, sjekke og sette opptatt (og kvis hvem som venter), slippe mutex, returneere ressurs 
    //får har det vært førstemann til mølla, det fikser vi: lage allocate som skal bestemme hvem som får ressurs etter prio. 

    T allocate(int priority){
        mtx.wait(); //en lås (mtx) for å låse system
        
        if (busy) { //opptatt
            //legg i kø
            numWaiting[priority]++;//øker teller til kø
            mtx.notify(); //slipp lås, typ på ut av rom 
            sems[priority].wait(); //legger deg inn i kø (vent) avhengig av prioriteten din 
        } else { //ledig
            busy = true; //sett opptatt/ta
            mtx.notify(); // slipp lås 
        }

        return value; //ressurs 
    }
    //Legge ressurs tilbake, låse tilgang, sjekke om noen venter, gi ressurs til bruere ut ifra prioritet. 
    //deallocate bestemmer hva som skjer med ressurs når en er ferdig - gi til enste person, velg riktig, sørg for at bare èn bruker ressurs 
    
    void deallocate(T v){
        value = v; //lagre ressurs
        mtx.wait(); //lås

        //velger hvem som skal få ressurs 
        if (numWaiting[1] > 0) { //hvis det er høy prio i kø, velg den 
            numWaiting[1]--;
            sems[1].notify(); //vekk tråden i kø
        } else if (numWaiting[0] > 0) { //hvis noen i kø med lav prio, velg den
            numWaiting[0]--;
            sems[0].notify();
        } else { //ingen i kø
            busy = false; //marker som ledig 
        }
        mtx.notify(); //slipper lås etter deallokert 
    }
}
